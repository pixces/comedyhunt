...
class Content extends CActiveRecord
{
    ...
    public static $defaultSelectableFields = ['id', 'username', 'title', 'thumb_image', 'media_id', 'media_url', 'is_ugc'];
	/**
	...
	public function rules()
	{
		...
		return array(
			array('username, email, title, description, type','required'),
			array('is_ugc', 'numerical', 'integerOnly'=>true),
			array('username, author, channel_name', 'length', 'max'=>150),
			array('email, title, media_url, thumb_image, alternate_image', 'length', 'max'=>255),
			...
			array('id, username, email, title, description, media_id, media_url, type, author, channel_name, is_ugc, thumb_image, alternate_image, status, date_created, date_modified', 'safe', 'on'=>'search'),
		);
	}
    ...
    public function beforeSave()
    {
        ...
        if ($this->isNewRecord) $this->date_created = new CDbExpression('NOW()');
